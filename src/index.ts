...
5
app.get("/quantidade_produtos_por_clientes", async (req, res) => {
    try {
        const [resultado, campos] =
            await connection.execute(`SELECT 
        c.nome, p.idpedidos AS idpedido, SUM(ip.quantidade) AS quantidade_produtos FROM clientes c JOIN pedidos p ON c.idclientes = p.clientes_idclientes JOIN itenspedidos ip ON p.idpedidos = ip.pedidos_idpedidos GROUP BY p.idpedidos, c.nome  `)
        console.log(resultado)
        res.status(200).json(resultado)
    } catch (err) {
        const mysqlErrorHandle = new MysqlErrorHandle(err, res)
...
app.get("/valor_pedido_total", async (req, res) => {
    try {
        const query = `SELECT c.nome, SUM(ip.quantidade * prod.preco) AS valor_total FROM clientes c JOIN pedidos p ON c.idclientes = p.clientes_idclientes JOIN itenspedidos ip ON p.idpedidos = ip.pedidos_idpedidos JOIN produtos prod ON ip.produtos_idprodutos = prod.idprodutos GROUP BY p.idpedidos, c.nome`;
